...
class WorkflowApiTests(unittest.TestCase):
    ...
    def test_fixture_execution_is_default_and_real_requires_explicit_flag(self) -> None:
        original = os.environ.pop("QWENPAW_WORKFLOW_EXECUTOR", None)
        try:
            service = WorkflowApiService(app.connect, app.now_iso)
            self.assertEqual(service.executor.__func__, service._fixture_executor.__func__)
            os.environ["QWENPAW_WORKFLOW_EXECUTOR"] = "real"
            live_service = WorkflowApiService(app.connect, app.now_iso)
            self.assertIsInstance(live_service.executor, WorkflowExecutor)
        finally:
            if original is None:
    ...
    def test_mining_route_generates_case_id_and_completes_miner(self) -> None:
        ...
        validate_schema(result, "SuccessfulResult")
        self.assertEqual(result["shop_id"], "lei-kei-001")
    ...
    def test_only_the_shared_demo_shop_is_accepted(self) -> None:
        ...
        self.assertEqual(payload["error"], "const")
