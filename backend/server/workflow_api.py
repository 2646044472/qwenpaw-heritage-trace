...
class WorkflowApiService:
    ...
    def handle_post(self, handler, path: str) -> bool:
        ...
        try:
            ...
            request, route, case_id = self._validate_request(payload)
        ...
        with closing(self.connect()) as db, db:
            db.execute("INSERT INTO heritage_workflow_runs (run_id, case_id, route, state, request_json, created_at, updated_at) VALUES (?, ?, ?, 'input_received', ?, ?, ?)", (run_id, case_id, route, json.dumps(request, ensure_ascii=False), timestamp, timestamp))
    ...
    def _validate_request(self, payload: object) -> tuple[dict, str, str]:
