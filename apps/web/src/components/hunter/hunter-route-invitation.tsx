...
function getInvitationCopy(shop: HunterShopProjection, isPlanning: boolean, isAdded: boolean) {
  if (isPlanning) {
    return {
      invitationLabel: "正在重新規劃路線",
      heading: "正在替你重新規劃路線",
      detail: `正在調整 ${shop.name} 的行程位置。`,
      actionLabel: "重新規劃路線…",
    };
  }
  if (isAdded) {
    return {
      invitationLabel: `已加入行程：${shop.name}`,
      heading: "這一站已在今天行程內",
      detail: `${shop.name} · ${shop.area}，如要調整可移除這一站並重新規劃。`,
      actionLabel: "移除並重新規劃路線",
    };
  }
  return {
    invitationLabel: `發現附近文化店舖：${shop.name}`,
    heading: "發現附近的文化店舖",
    detail: `${shop.name} · ${shop.area}，留 ${shop.visitMinutes} 分鐘，順路吃一口老澳門。`,
    actionLabel: "加入行程",
  };
}
...
export function HunterRouteInvitation({
  ...
  const { invitationLabel, heading, detail, actionLabel } = getInvitationCopy(shop, isPlanning, isAdded);
  return (
    <section
    ...
    >
      ...
      <div className="flex gap-3 pr-7">
        ...
        <div className="min-w-0">
          ...
          {!isPlanning && !isAdded ? <p className="mt-1 text-[#e0bc72] text-[11px]">已核實的文化店舖</p> : null}
        </div>
      </div>
      <button
