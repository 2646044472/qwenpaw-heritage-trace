...
Heritage Trace is a responsive Next.js 16 application built from the Studio Admin foundation with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui. It presents Government, Merchant, and Hunter surfaces over one shared Heritage Trace data model.

Read the workspace-level `../Implementation.md` before broad implementation. Treat it as the source of truth for product scope, architecture, routes, data flow, and acceptance criteria.

## Heritage Workflow contract

- The frontend integrates only with the public Workflow API through an adapter.
- `contracts/heritage-workflow.openapi.yaml` is the canonical OpenAPI 3.1 contract.
- Generate TypeScript workflow types into `src/lib/heritage/generated/workflow-types.ts`; never hand-edit generated files or maintain duplicate handwritten workflow types.
- `RevisedAssetCard` is the authoritative heritage asset. Do not create a parallel `HeritageAsset` model.
- Handle `WorkflowResult` as a discriminated union on `workflow_status`.
- `HeritageShop.workflow` contains only `SuccessfulResult`.
- Never import, declare, or depend on backend-internal agent types, including raw Miner/Archivist/Verifier outputs, `claims[]`, `story_claims[]`, `claim_verifications[]`, and internal normalized source bundles.
- Keep mock or locally derived `ShopSignals`, `ShopInsight`, `HeritageShop`, and demo state separate from the generated Workflow API contract.
